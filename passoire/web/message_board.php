...
if (isset($_GET['filter']) && strlen($_GET['filter']) > 0) {
	...
	$stmt = $conn->prepare("SELECT m.id, m.content, m.date, u.login, i.avatar FROM messages m JOIN users u ON m.authorid = u.id JOIN userinfos i ON u.id = i.userid WHERE u.login = ? ORDER BY date DESC, id DESC LIMIT ? OFFSET ?");
	$stmt->bind_param("sii", $filter, $limit, $offset);
} else {
	$stmt = $conn->prepare("SELECT m.id, m.content, m.date, u.login, i.avatar FROM messages m JOIN users u ON m.authorid = u.id JOIN userinfos i ON u.id = i.userid ORDER BY date DESC, id DESC LIMIT ? OFFSET ?");
...
<?php if (isset($_SESSION['user_id'])): ?>
	<div class="w3-container w3-card w3-white w3-round w3-margin-right w3-margin-left w3-margin-bottom"><br>
		<form action="index.php" method="post">
			...
			<textarea name="content" contenteditable="true" class="w3-border w3-padding"
				style="width: 100%; box-sizing: border-box;" required>Your message.</textarea><br />
...
<?php foreach ($messages as $msg): ?>
	<div class="w3-container w3-card w3-white w3-round w3-margin"><br>
	<img src="<?php echo htmlspecialchars(!empty($msg['avatar']) ? $msg['avatar'] : 'default_avatar.png', ENT_QUOTES); ?>" alt="Avatar" class="w3-left w3-circle w3-margin-right" style="width:60px">
...
<div class="w3-container w3-card w3-white w3-round w3-margin">
	...
	<?php for ($i = 1; $i <= $totalPages; $i++): ?>
		<a href="index.php?page=<?php echo $i; ?>&limit=10"
